...
def get_inception_and_fid_score(images, fid_cache, num_images=None,
    ...
    if use_torch:
        ...
        print(np.any(np.isnan(m1)), np.any(np.isinf(m1)))
        print(np.any(np.isnan(s1)), np.any(np.isinf(s1)))
        print(np.any(np.isnan(m2)), np.any(np.isinf(m2)))
        print(np.any(np.isnan(s2)), np.any(np.isinf(s2)))
    else:
